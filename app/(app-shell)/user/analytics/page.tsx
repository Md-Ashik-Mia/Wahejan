...
function normalizeApiAnalytics(raw: unknown): NormalizedAnalytics {
  ...
  const platforms = isRecord(messageCount.platforms)
    ? messageCount.platforms
    : {};

  const channelMessages = isRecord(r.channel_messages)
    ? r.channel_messages
    : {};
  ...
  return {
    ...
    channel_messages: Object.fromEntries(
      Object.entries(channelMessages).map(([k, v]) => [
        k.toLowerCase(),
        toNumber(v, 0),
      ]),
    ),
...
function normalizeQuestionLeaderboard(
  payload: unknown,
...
const AnalyticsDashboard: React.FC = () => {
  ...
  const fetchAnalytics = useCallback(
    async (range: string, ch: ChannelFilter) => {
      setLoading(true);
      setError(null);
      ...
      const channelParam =
        ch === "All Channels" ? "" : String(ch).trim().toLowerCase();
      ...
      const candidates: Array<{
        ...
        {
          endpoint: "/analytics/",
          params: { ...baseParams, time: "this_year" },
        },
      ];
      ...
      try {
        ...
        const channelDistribution = channels
          .filter((n) => n !== "All Channels")
          .map((name) => {
            const key = name.toLowerCase();
            const value =
              typeof platforms[key] === "number" ? platforms[key] : 0;
  ...
  const fetchFinance = useCallback(async () => {
    ...
    const candidates = ["/finance-data/", "/finance-data"];
  ...
  }, [fetchAnalytics, isBlocked, sessionStatus, timeRange, channel]);
  ...
  useEffect(() => {
    if (sessionStatus === "loading" || isBlocked) return;
    void fetchFinance();
  }, [fetchFinance, isBlocked, sessionStatus]);
        ...
        <div>
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (TIME_RANGES.includes(v as TimeRange))
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            {[
              ...
              <div
                key={i}
                className="bg-[#272727]  rounded-lg p-4 text-center shadow-lg"
              >
          ...
          {isBlocked ? (
            <div className="bg-[#272727] rounded-lg p-6 text-center">
              <p className="text-gray-400">
                Financial insights are not available for your account.
              </p>
            </div>
          ) : (
            <section>
              ...
              {financeError ? (
                <p className="text-sm text-red-400 mt-2">{financeError}</p>
              ) : null}
              ...
              <div className="mt-4 grid grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-[#272727] rounded-xl p-5 shadow-lg">
                  ...
                  <p className="text-2xl font-bold mt-4">
                    {formatCurrency(data.financial.totalPayments, 0)}
                  </p>
                  <p className="text-gray-400 text-sm">Total Payments</p>
                  <p className="text-xs mt-1 text-green-400">
                    {data.financial.growth.payments}
                  </p>
                </div>
                ...
                <div className="bg-[#272727] rounded-xl p-5 shadow-lg">
                  ...
                  <p className="text-2xl font-bold mt-4">
                    {formatCurrency(data.financial.avgOrder, 0)}
                  </p>
                  <p className="text-gray-400 text-sm">Average Order Value</p>
                  <p className="text-xs mt-1 text-green-400">
                    {data.financial.growth.avgOrder}
                  </p>
                </div>
                ...
                <div className="bg-[#272727] rounded-xl p-5 shadow-lg">
                  ...
                  <p className="text-2xl font-bold mt-4">
                    {typeof data.financial.failedPayments === "number"
                      ? data.financial.failedPayments.toLocaleString()
                      : "—"}
                  </p>
                ...
                <div className="bg-[#272727] rounded-xl p-5 shadow-lg">
                  ...
                  <p className="text-2xl font-bold mt-4">
                    {typeof data.financial.pendingPayments === "number"
